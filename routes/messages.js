...
router.post("/sendMessages", (req, res) => {
    ...
    if (!username || !message || !chatId) {
        res.send({ success: false, error: "Username, message, or chatId not supplied" });
        return;
    }
    // TODO: Check that the user is part of the chat
...
router.get("/getMessages", (req, res) => {
    ...
    ORDER BY Timestamp ASC`
    db.manyOrNone(query, [after, chatId]).then((rows) => {
        res.send({ messages: rows })
    }).catch((err) => {
        res.send({
            success: false, error: err
